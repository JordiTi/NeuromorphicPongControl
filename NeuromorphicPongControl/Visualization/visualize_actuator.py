...
for j in range(iterations):
    ...
    mf_spikehist = np.zeros([int(outputsize/2), int(width*1.25)])
    mf_anspikehist = np.zeros([int(outputsize/2), int(width*1.25)])
    while not reset:
        t += dt
        ...
        mf_spikehist[:,t] = outputspikesl3[0:int(outputsize/2)]
        mf_anspikehist[:,t] = outputspikesl3[int(outputsize/2):int(outputsize)]
        aghist[:,t] = mf_agonist.fibers_c2.copy()
        anhist[:,t] = mf_antagonist.fibers_c2.copy()
        # Move paddle
        ...
        if reset:
            ...
            indicesag = []
            indicesan = []
            colorsag = ["blue", "orange"]
            colorsan = ["green", "red"]
            for index, a in enumerate(aghist):

                if np.sum(a)>1:
                    print(np.sum(a)>50)
                    if cntag < 2:
                        color = colorsag[cntag]
                        plt.plot(a, color=color)
                        plt.vlines(np.where(mf_spikehist[index, :] > 0), 0, 0.05, color=color, linewidth=2)
                        cntag += 1
                        indicesag.append(index)

            for index, a in enumerate(anhist):
                if np.sum(a) > 1:
                    if cntan < 2:
                        color = colorsan[cntan]
                        plt.plot(-a, color=color)

                        plt.vlines(np.where(mf_anspikehist[index, :] > 0), 0, -0.05, color=color, linewidth=2)
                        cntan += 1
                        indicesan.append(index)
            plt.plot(aghist.T, alpha=0.3)
            plt.plot(-anhist.T, alpha=0.3)

            lastvalue = np.where(np.sum(aghist + anhist, axis=0)>0)[-1]
            plt.ylim([-0.4, 0.4])
            plt.xlim([0, lastvalue[-1]])
            plt.show()
            assert()
            break
